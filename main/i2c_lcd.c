...
void lcd_send_cmd(uint8_t lcd_addr, char cmd)
{
    ...
    lcd_err = i2c_master_write_to_device(I2C_NUM, lcd_addr, data_t, 4, 1000);
    
    // Log an error message if there is an error in sending the command
    if (lcd_err != 0) ESP_LOGI(TAG, "Error in sending command");
}
...
void lcd_send_data(uint8_t lcd_addr, char data)
{
    ...
    if (lcd_err != 0) ESP_LOGI(TAG, "Error in sending data");
}
